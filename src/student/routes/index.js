...
import {Root, Auth, Lesson, BuyPresent, RegistrationSale, SamsungLesson, SamsungPresentation, Quiz, QuizPlusPLus, Account, SelectLesson, IndexPage, updateProfile, RestorePassword} from '../pages';
...
//     }
// };
const lessonPlusSelectedAndFetched = UserAuthWrapper({
    authSelector: state => state.dic, // how to get the user state
    failureRedirectPath: '/samsungLesson',
    predicate: dic => (!!dic.isLoadedLessons),
    redirectAction: routerActions.replace, // the redux action to dispatch for redirect
    wrapperDisplayName: 'lessonSelectedAndFetched' // a nice name for this auth check
});
const presentationLoaded = UserAuthWrapper({
    authSelector: state => state.samsungPlusPlus, // how to get the user state
    failureRedirectPath: '/samsungLesson',
    predicate: samsungPlusPlus => (!!samsungPlusPlus.presentationUrl),
    redirectAction: routerActions.replace, // the redux action to dispatch for redirect
    wrapperDisplayName: 'presentationLoaded' // a nice name for this auth check
});
...
export default(
    ...
    <Route name="Root" path='/' component={Root}>
        ...
        <Route name="quizPlusPlus" path='quizPlusPlus' >
            {/*temp mock redirect */}
            <Route name="question" path=":id" component={lessonPlusSelectedAndFetched(QuizPlusPLus)} />
            <IndexRedirect to="1"/>
        </Route>
        ...
        <Route name="samsungLesson"  path="samsungLesson" component={SamsungLesson} />
